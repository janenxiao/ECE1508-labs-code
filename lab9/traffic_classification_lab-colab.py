...
# In the following lines, you will find the functions that has been used to 
# calculate the features of each flow and stored in the feature matrix.
    ##### Since the structure of the second file is different than the first 
...
# Here we read the ground truth csv file and we reorganize into a new dataframe called nvgt
gt = pandas.read_csv('/content/drive/My Drive/Colab Notebooks/ECE1508_lab9/gt.csv')
gt2 = gt[gt.columns[0]].values.tolist()
# print(gt[gt.columns[0]].head(10))   # print first 10 rows
gt3 = [s.split(':') for s in gt2]
# print(gt3[0:2])   # print first 2 list items
nvgt = pandas.DataFrame(gt3)
# print(nvgt.head(10))

nvgt.columns = ['TimeStamp','IP src','IP dst','Port src','Port dst','DPI','App','Protocol']

# We create the column 'mydata_MERGED' that will characterize each flow by its source and destination IP adresses, 
# source and destination port numbers and protocol.
nvgt['mydata_MERGED'] = (nvgt['IP src'] + '>' + nvgt['IP dst'] + '>'+ nvgt['Port src'] + '>'+nvgt['Port dst']+'>'+ nvgt['Protocol']).map(lambda x: sort_mydata(x.split('>')))

nvgt.sort_values(by=['TimeStamp'], inplace=True)
...
mydata2 = pandas.read_csv('/content/drive/My Drive/Colab Notebooks/ECE1508_lab9/file2_2')   # nrows=99

# Extract port number from the Info field
...
# We create the column 'mydata_MERGED' that will characterize each flow by it source and destination IP adresses,
# source and destination port numbers and protocol.
...
mydata2.sort_values(by=['Time'], inplace=True)
...
# Feature : Inter arrival time - the time between the arrive times of two packets to the network
...
# print(mydata_selected.groupby('mydata_MERGED')[columns].sum())
...
# The next step consists on merging the feature table with ground truth in order to add a new column called label.
...
# Read the csv file containing the traffic traces and store it in a dataframe called mydatatest
#mydatatest = pandas.read_csv('file3_3')
mydatatest = pandas.read_csv('/content/drive/My Drive/Colab Notebooks/ECE1508_lab9/file3_3', engine='python', error_bad_lines=False)    # nrows=99
...
#SACK number: SACKs allow a receiver to acknowledge non-consecutive data, so that the sender can retransmit only what is missing at the receivers end.    
...
# *************************** Feature Importance  *********************************
##################################################################################
# In the next step, we will use Random Forest in order to select from all the features we calculated, the ones that
# are significant. This will reduce training times and provide better classification performances.
...
# avoid the problem of majority and minority classes. Here it is set at 100, you will vary this number
# according to the assignement in order to see the influence that it has on the performance of classification algorithms.
...
lst_lb = []
for cnt1 in range(6):
    d = dat[dat['Label']==cnt1].iloc[:numflowsInClass].copy(deep=True)
    print(d.shape)
    lst_lb.append(d)
dat2 = pandas.concat(lst_lb)
print('dat2 shape: ', dat2.shape)

# Print the total number of flows in each class
for cnt1 in range(6):
    d = dat[dat['Label']==cnt1]
    print(d.shape)
...
arrayy = dat2.values    # return a Numpy representation of the DataFrame
Xy = arrayy[:,0:28]     # The features (a total of 28 features)
...
print(clfy.feature_importances_)

modely = SelectFromModel(clfy, prefit=True)   # select features based on importance weights
X_new = modely.transform(Xy)      # reduce Xy to the selected features

# Here you can see the shape of the new dataframe that represents the features with significant information only.
X_new.shape

# print(Xy[:5])
# print(Xy[np.ix_(np.arange(5), modely.get_support())])   # first 5 rows in Xy, columns are the selected features
# print(X_new[:5])    # should be equivalent as above
# modely.get_support()
...
# Training and classification using several models
from sklearn.tree import DecisionTreeClassifier
from sklearn.ensemble import RandomForestClassifier 
from sklearn.ensemble import BaggingClassifier
import time
...
# Ensemble methods consist of training different algorithms with several samples, then combining using a certain method
# approach the obtained classifiers into one improved model. Several ensemble strategies exist.
# Voting, consists of applying several ML algorithms on samples. The experience results in a group of labels and 
...
# Training the MLP model (MultiLayer Perceptron) using X_train and Y_train
...
# Bagging, also called Boostrap algorithm consists of iteratively extracting 
# groups of m samples from the training set, T times resulting in T bootstrap 
# samples B1,B2,B3,…,BT, and training T classifier C1,C2,C3,…,CT.
...
# algorithm consists of converting weak inducers to stronger ones by changing 
...
# We will test whether an unsupervised ML algorithm is more efficient than supervised MLs, and whether 
...
scaled_features = scaler.fit_transform(Xy)    # standardize features by removing the mean and scaling to unit variance
...
kmeans.fit(scaled_features)   # compute k-means clustering

# The number of iterations required to converge
kmeans.n_iter_
print("Number of kmeans labels: ", len(np.unique(kmeans.labels_)))
...
# ARI is a function that measures the similarity of between the ground truth and the predictions given by clustering
# ARI is 0 for random labeling, and 1 when the predictions are perfect
ari1 = adjusted_rand_score(Yy, kmeans.labels_)

# Silhouette Coefficient is calculated using the mean intra-cluster distance (a) (distance among members of a cluster)
# and the mean nearest-cluster distance (b) (distance to a member of another cluster) for each sample.
# Silhouette Coefficient for a sample = (b - a) / max(a, b). The best value is 1 (a << b, sample is close to members in its own cluster,
# and far from members in other clusters) and the worst value is -1. Values near 0 indicate overlapping clusters.
# Negative values generally indicate that a sample has been assigned to the wrong cluster, as a different cluster is more similar.
silhouette1 = silhouette_score(Xy, kmeans.labels_).round(2)

### Print ARI and silhouette scores
print("ARI: ", ari1)
print("Silhouette: ", silhouette1)

### Plot correlation matrix (using all the features, we have removed the label column)
f, ax = plt.subplots(figsize=(10, 8))
corr = dat[dat2.columns[0:28]].corr()   # pairwise correlation of columns
...
print("Number of kmeans labels: ", len(np.unique(kmeans_new.labels_)))
print(classification_report(Yy, kmeans_new.labels_))
...
Xnew_columns = dat2.columns.to_numpy()[0:28][modely.get_support()]
dataset = pandas.DataFrame(X_new, columns=Xnew_columns)
...
n_clusters=6
# Pipeline chains a fixed sequence of steps in processing the data, for example feature selection, normalization and classification
preprocessor = Pipeline(
    [
        ("scaler", MinMaxScaler()), # Scale the value of each feature to [0, 1]
        ("pca", PCA(n_components=10, random_state=42)), # Linear dimensionality reduction using Singular Value Decomposition of the data to project it to a lower dimensional space
    ]
)
clusterer = Pipeline(
   [
...
# For a fixed n_components in PCA
# pipe.fit(Xy)

# preprocessed_data = pipe["preprocessor"].transform(Xy)
# # print(preprocessed_data.shape)
# print(pipe["preprocessor"]["pca"].components_.shape)

# predicted_labels = pipe["clusterer"]["kmeans"].labels_
# print("Number of kmeans labels: ", len(np.unique(predicted_labels)))

# silhouette3=silhouette_score(preprocessed_data, predicted_labels)
# ari3=adjusted_rand_score(Yy, predicted_labels)

# print("ARI: ", ari3)
# print("Silhouette: ", silhouette3)

# Vary n_components in PCA
for num_components in range(3, 28, 3):
    pipe["preprocessor"].set_params(pca__n_components=num_components)
    pipe.fit(Xy)
    preprocessed_data = pipe["preprocessor"].transform(Xy)
    print("data shape: ", preprocessed_data.shape)
    print("components_ shape: ", pipe["preprocessor"]["pca"].components_.shape)
    predicted_labels = pipe["clusterer"]["kmeans"].labels_
    print("Number of kmeans labels: ", len(np.unique(predicted_labels)))
